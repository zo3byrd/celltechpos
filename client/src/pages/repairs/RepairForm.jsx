...
<div class="sig-grid">
  ...
  function printRepairLabel() {
    ...
    const custName = cust ? `${cust.firstName} ${cust.lastName}` : '—';
    const custPhone = cust?.phone || '';
    const device = [ticket.deviceBrand, ticket.deviceModel, ticket.color].filter(Boolean).join(' ');
    const date = new Date(ticket.createdAt).toLocaleDateString('en-US', { month:'2-digit', day:'2-digit', year:'2-digit' });
    const status = (ticket.status || '').replace(/_/g, ' ').toUpperCase();
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .store{font-size:7.5pt;font-weight:bold;text-transform:uppercase;letter-spacing:.04em;margin-bottom:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  ...
  .lbl{font-size:6.5pt;font-weight:bold;text-transform:uppercase;color:#666;display:block;margin-bottom:0}
...
<div class="tnum">${ticket.ticketNumber}</div>
...
${ticket.imei ? `<div class="row"><span class="lbl">IMEI / Serial</span><span class="val">${ticket.imei}</span></div>` : ''}
